...
`/spdd-story` is the optional Phase 0 of the canonical SPDD workflow. It creates business-level epics or stories in `requirements/` for processing by `/spdd-analysis`; the shared lifecycle, artifact handoffs, and phase numbering are defined in the root `AGENTS.md`.